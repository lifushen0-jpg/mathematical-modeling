...
这个脚本的目标是：
- 以圆柱为真实目标对象，计算云团在什么时间段内会“严密遮住”整根圆柱；
- 同时和点模型对照，验证圆柱效应是否会导致更长/更短的有效遮蔽时长；
- 采用多种采样密度 + 最坏圆周复核，得到最终推荐值。
"""
...
# 让仓库根目录可被直接 import，以便使用 common/smoke_geom 中的通用几何函数
...
def main():
    ...
    # 2) 圆柱模型：使用不同采样密度测试，评估稳态与计算速度的折中
    for name, pts, n in [
    ...
    # 将起止边界整理为区间列表：[(start1, end1), (start2, end2)]
    segs_c = []
    state = bool(ok[0])
    cur = t_lo
    for e in edges:
    ...
    with open(txt, "w", encoding="utf-8") as f:
        ...
        for k in ("cyl_fast", "cyl_opt", "cyl_fine", "cyl_ultra"):
            r = results[k]
            f.write(f"{k}: {r['duration']:.6f} s  n_pts={r['n_pts']}  segs={r['segments']}\n")
