...
这个脚本和 q1_refine.py 的思路相同，但重点是利用 NumPy 向量化一次性处理大量时间点。
其目的是：
- 尽量保持几何公式不变；
- 直接批量计算所有时间点上的 d 和 s；
- 用于快速得到全局遮蔽时长和边界时刻；
...
def dist_and_s_batch(ts, T):
    """向量化批量求解：对每个时间戳 t，计算云团中心到导弹-目标线段的距离 d 和参数 s。

    这里使用批量数组操作，避免 for 循环带来的性能损耗。
    """
...
def duration_for_target(T, n=2_000_001):
    """对某个目标 T 计算有效遮蔽时长，并用二分法细化进入/离开时刻。"""
    t0 = t_det
...
def full_cylinder_duration(n_time=400001, n_theta=48, n_z=9):
    """对圆柱全部采样点同时被遮挡的总时长进行计算。

    注意：这是一种“严密判定”的近似版本，要求所有采样点在同一时刻都满足遮挡条件。
    """
    pts = []
    Rc, H = 7.0, 10.0
    for iz in range(n_z):
