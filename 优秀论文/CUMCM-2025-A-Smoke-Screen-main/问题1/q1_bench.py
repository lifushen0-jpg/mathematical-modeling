# -*- coding: utf-8 -*-
"""问题1 基准测试脚本：用于对比 NumPy 向量化实现和纯 Python 实现的速度。

作用：
- 只关注给定策略下的几何判定是否命中（云团与导弹-目标连线的距离判定）；
- 统计“有效遮蔽时长”的计算量和运行时间；
- 可作为性能基准，验证后续优化脚本是否更快。
"""
import time
import numpy as np
...
def run(n):
    """向量化版：一次性计算大量时间点上是否遮挡。

    采用点目标判定：
    1）导弹位置 M(t)
    2）云团中心 C(t)
    3）计算 C 到线段 M(t)-T 的最近距离 d，以及投影参数 s
    4）当 s ∈ [0,1] 且 d <= R 时，认为在该时刻被遮挡
    """
...
def run_python(n):
    """纯 Python 版本，用于验证 NumPy 向量化方案的速度提升。"""
    t0 = time.perf_counter()
...
if __name__ == "__main__":
    ...
    # 4 个目标 + 圆柱扫描的综合压力测试：用于估算“真实场景”规模下的耗时
