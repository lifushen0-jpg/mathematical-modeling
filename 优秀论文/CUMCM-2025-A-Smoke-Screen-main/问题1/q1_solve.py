...
这个脚本是“结果计算器”的版本，强调：
- 直接扫描时间区间，得到每个判定量的有效遮蔽时长；
- 记录各个代表点、圆柱采样点的遮蔽区间；
- 输出文本日志，方便后续整理成论文中的计算结果。
"""
import math
...
def main():
    """主流程：
    1）根据物理参数构造导弹和云团运动轨迹；
    2）计算投放点、起爆点和导弹抵达假目标的时间；
    3）逐项计算点目标和圆柱采样点的遮蔽时长；
    4）输出文本结论，供定稿时使用。
    """
    g = 9.8
    v_drone = 120.0
